...
export const validarMensajeChat = [
    ...
    body('historial.*.contenido').isString().isLength({ min: 1, max: 4000 }).withMessage('contenido del historial inválido'),
    body('historial.*.firma').optional().isString().isLength({ max: 100 }).withMessage('firma del historial inválida'),
];
